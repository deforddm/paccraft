...
(function () {
  ...
  function drawTitleScene(dt) {
    ...
    var period = 9, t = titleT % period, chase = t < period / 2;
    var prog = chase ? t / (period / 2) : (t - period / 2) / (period / 2);
    var span = c.width + 110, hx = chase ? -20 + prog * span : c.width + 20 - prog * span;
    var set = PCTex.hero(save.look), frame = 1 + Math.floor(titleT * 8) % 2;
    // gems ahead of the hero on the chase leg
    if (chase) for (var gx = 8; gx < c.width; gx += 14) if (gx > hx + 8) { g.fillStyle = '#5ef0a8'; g.fillRect(gx, 22, 3, 3); }
    if (chase && hx < c.width - 30) g.drawImage(tex.crystal, c.width - 24, 13, 9, 11);
    g.drawImage(chase ? set.right[frame] : set.left[frame], Math.round(hx) - 8, 14, 16, 16);
    ['rumble', 'sly', 'frost', 'mudge'].forEach(function (k, i) {
      var mx = chase ? hx - 26 - i * 19 : hx + 30 + i * 19;
      var hop = Math.abs(Math.sin(titleT * 9 + i)) * 3;
      var kind = chase ? k : (Math.floor(titleT * 6) % 2 && prog > 0.8 ? 'flash' : 'fright');
  ...
  window.PacCraft = { save: function () { return save; }, session: function () { return session; }, show: show, startAdventure: startAdventure, renderer: renderer, VERSION: VERSION };
})();
